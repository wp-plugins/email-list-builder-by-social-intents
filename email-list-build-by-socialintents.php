...
http://www.socialintents.com/" title="', '">', '</a>') ?></p>
		...
		if(get_option('elb_header_text') && get_option('elb_header_text') != '') {
   		...
   		?>
		<input type="text" name="elb_header_text" id="elb_header_text" value="There you are - we\'ve been waiting for you!" style="margin:3px;width:100%;" />
		<?php 
			}
   		?>
		</td></tr>
...
var elb_wid= $('#elb_widgetID').val();
var elb_tt= $('#elb_tab_text').val();
var elb_ht= $('#elb_header_text').val();
...
$.ajax({
   type: 'GET',
    ...
    <?php }
    add_submenu_page('options-general.php', __('Social Intents', $elb_domain), __('Social Intents', $elb_domain), 'manage_options', 'email-list-builder-by-socialintents', 'elb_settings_page');
}?>
